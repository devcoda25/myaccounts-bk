...
export class UserDisputesController {
    ...
    @Post()
    async createDispute(@CurrentUser() user: AuthRequest['user'], @Body() body: any) {
        return this.service.createDispute(user.sub || (user as any).id, body);
    }

    @Get()
    async getMyDisputes(@CurrentUser() user: AuthRequest['user']) {
        return this.service.getMyDisputes(user.sub || (user as any).id);
    }

    @Post(':id/evidence')
    async addEvidence(@CurrentUser() user: AuthRequest['user'], @Param('id') id: string, @Body() body: any) {
        return this.service.addEvidence(user.sub || (user as any).id, id, {
